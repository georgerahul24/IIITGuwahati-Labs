...
class State:
    ...
    def h(self):
        ...
        numberOfChecksWithOtherQueens = 0
        for queen1 in self.queens:
            for queen2 in self.queens:
                if (_checkChecks(queen1, queen2)):
                    numberOfChecksWithOtherQueens += 1
        return numberOfChecksWithOtherQueens
...
while True:
    ...
    if possibleState is not None and len(possibleState) > 0:
        for state in possibleState:
            genState = State(_newState.depthLevel + 1, state)
            genState.f()
            openList.append(genState)
